...
<?php switch ($data['status']) {
    ...
    case 2:
        ?>
        <span style="color:red">Пользователь не активирован! Для активации пользователя перейдите по ссылке указанной в письме, полученом Вами при регистрации</span>
        <br>
    ...
    <?php endif; ?>
        ...
        <div class="create-user-account-form">
            <p class="custom-text">В своем кабинете Вы сможете следить за статусами Ваших заказов, так же изменять свои
                личные данные.</p>
            ...
            <div class="personal-tabs">
                ...
                <div id="personal">
                    ...
                    <form action="<?php echo SITE ?>/personal" method="POST">
                        <ul class="form-list">
                            ...
                            <li>Дата регистрации: <span
                                        class="normal-text"><?php echo date('d.m.Y', strtotime($userInfo->date_add)) ?></span>
                            </li>
                        </ul>
                        <ul class="form-list">
                            <li>Имя:</li>
                            ...
                            <li><textarea class="address-area"
                                          name="address"><?php echo $userInfo->address ?></textarea></li>
                            <li>
                        ...
                        <?php
                        /* Вставляем дополнительные поля пользователя
                        из файла layout/layout_user_opt_fields.php */
                ...
                <div id="change-pass">
                    ...
                    <form action="<?php echo SITE ?>/personal" method="POST">
                        <p class="custom-text"><span class="red-star">*</span>Поля отмеченные красной звездочкой,
                        ...
                        <button type="submit" class="save-btn default-btn" name="chengePass" value="save">Сохранить
                        </button>
                        <div class="clear"></div>
                    </form>
                </div>
                        ...
                        <div class="order-history-list">
                            ...
                            foreach ($data['orderInfo'] as $order):
                                ...
                                <div class="order-history" id="<?php echo $order['id'] ?>">
                                    <div class="order-number">
                                        ...
                                        <span class="order-status"> Cтатус заказа: <span
                                                    class="<?php echo $data['assocStatusClass'][$order['status_id']] ?>"><?php echo $lang[$order['string_status_id']] ?></span></span>
                                    </div>
                                    <table class="status-table">
                                        ...
                                        <?php if (!empty($perOrders)) foreach ($perOrders as $perOrder): ?>
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo priceFormat($perOrder['price']) . '  ' . $perCurrencyShort; ?></td>
                                                <td><?php echo $perOrder['count'] ?> шт.</td>
                                                <td><?php echo priceFormat($perOrder['price'] * $perOrder['count']) . '  ' . $perCurrencyShort; ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </table>
                                    <?php if (($order['status_id'] == 2 || $order['status_id'] == 5) && MG::getSetting('useElectroLink') != 'false'): ?>
                                        <a href="<?php echo SITE . '/order?getFileToOrder=' . $order['id'] ?>"
                                           class="download-link">
                                            Скачать электронные товары
                                        </a>
                                    <?php endif; ?>
                                    <?php $yurInfo = unserialize(stripslashes($order['yur_info']));
                                    if (1 || !empty($yurInfo['inn'])): ?>
                                        <a href="<?php echo SITE . '/mg-admin?getOrderPdf=' . $order['id'] ?>"
                                           class="download-link">
                                            Скачать счет в PDF
                                        </a>
                                    <?php endif; ?>
                                    ...
                                    <?php if (2 > $order['status_id']): ?>
                                        <div class="order-settings">
                                            <form method="POST" action="<?php echo SITE ?>/order">
                                                ...
                                                <button type="submit" name="pay" value="go" class="default-btn">Оплатить
                                                    заказ
                                                </button>
                                            </form>
                                        </div>
                                    ...
                                    <?php if ($order['status_id'] < 2): ?>
                                        <div class="order-settings">
                                            <button class="close-order" id="<?php echo $order['id'] ?>"
                                            ...
                                            <br/>
                                            <button class="change-payment" id="<?php echo $order['id'] ?>"
                                                    date="<?php echo date('d.m.Y', strtotime($order['add_date'])) ?>"
                                                    href="#changePayment">
                                    ...
                                    <div class="order-total">
                                        <ul class="total-list">
                                            ...
                                            <?php endif; ?>
                                            <li>Итого:
                                                <span><?php echo $order['summ'] . '  ' . $perCurrencyShort ?></span>
                                            </li>
                                            ...
                                            <?php if ($order['delivery_cost']): ?>
                                                <li>Стоимость доставки :
                                                    <span><?php echo $order['delivery_cost'] . '  ' . $perCurrencyShort; ?></span>
                                                </li>
                                            <?php endif; ?>
                                            <li>Всего к оплате:
                                                <span><?php echo $totSumm . '  ' . $perCurrencyShort; ?></span></li>
                                        </ul>
                                    </div>
                                    <div class="clear">&nbsp;</div>
                                </div>
                            ...
                            <div class="close-reason">
                                ...
                                <div class="close-reason-wrapper" id="openModal">
                                    <p class="order-number">Закрытие заказа №
                                        <strong name="orderId"
                                                class="orderId"></strong>
                                        от <span class="orderDate"></span>
                                    </p>
                                ...
                                <div class="successful-closure" id="successModal">
                                    ...
                                    <p class="order-close-text">Заказ №<strong class="orderId"></strong> от <span
                                                class="orderDate"></span></p>
                                    <p class="order-close-text green-color">Был успешно отменен!</p>
                                    <p id="order-comm"></p>
                                    <a href="#" id="close-order-successbtn" onClick="$.fancybox.close();"
                                       class="default-btn">Выход</a>
                                    <div class="clear"></div>
                                </div>
                                <!--Эта часть появляется после закрытия заказа без перезагрузки страницы-->
                            ...
                            <div class="change-payment">
                                <div class="close-reason-wrapper" id="changePayment">
                                    <p class="order-number">Выберите способ оплаты для заказа №<strong name="orderId"
                                                                                                       class="orderId"></strong>
                                        от <span class="orderDate"></span></p>
